...
public class Furnace implements IModBlock {
    ...
    public BlockGenerator getBlockGenerator() {
        ...
        BlockGenerator.State state = generator.createBlockState("lit=off", "tech-mod:model_electric_furnace_off", false, "events", true);
        state.stateGenerators = new String[]{"puzzle-loader:rotated"};
        state.catalogHidden = true;
        state.allowSwapping = false;
        BlockGenerator.State state2 = generator.createBlockState("lit=on", "tech-mod:model_electric_furnace_on", false, "events", true);
        state2.stateGenerators = new String[]{"puzzle-loader:rotated"};
        state2.catalogHidden = true;
        state2.allowSwapping = false;
        generator.addBlockEntity(FurnaceBlockEntity.id.toString(), Map.of());
        return generator;
    }
